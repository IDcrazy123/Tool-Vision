...
class ConfigLayoutIntegrationTests(unittest.TestCase):
    ...
    def test_fresh_install_prepares_file_and_never_edits_machine_configs(self):
        ...
        target = self.config_dir / "Printer-Setup" / "tool_vision.cfg"
    ...
    def test_upgrade_copies_legacy_files_but_keeps_manual_layout_intact(self):
        ...
        legacy_dir.mkdir()
        legacy_cfg = "[tool_vision]\npin: ^PF2\n"
        (legacy_dir / "tool_vision.cfg").write_text(legacy_cfg, encoding="utf-8")
        (legacy_dir / "tool_vision_state.json").write_text(
            '{"schema_version": 2}\n', encoding="utf-8"
        )
        ...
        new_dir = self.config_dir / "Printer-Setup"
        self.assertEqual(
            (new_dir / "tool_vision.cfg").read_text(encoding="utf-8"), legacy_cfg
        )
